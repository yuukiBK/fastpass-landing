...
function generateDataPoints(cx: number, cy: number, scores: number[]): string {
  const points = [];
  for (let i = 0; i < 5; i++) {
    const angle = (i * 72 - 90) * (Math.PI / 180);
...
export default function ResultDetailPage() {
  ...
  const evaluation = {
    ...
    goodPoints: [
      '学生時代の経験を具体的な数字を交えて説明できている点が評価できます。',
      '自己分析が深く、自身の強みを一貫して伝えられています。',
    ],
    improvements: [
      '志望動機との接続がやや弱いため、なぜこの企業でその強みを活かしたいのかをより明確に伝えましょう。',
      '話すスピードがやや速く、重要なポイントが埋もれてしまう場面がありました。',
    ],
  };
  ...
  const chatMessages = [
    ...
    },
    {
      role: 'user',
      message:
        'はい、〇〇大学経済学部の山田太郎と申します。私の強みは「周囲を巻き込む力」です。大学ではテニスサークルの副代表として、新入生の集客に課題を感じ、SNS戦略を提案・実行しました。その結果、前年比200%の新入生獲得に成功しました。',
      feedback: {
    ...
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <button
            onClick={handleBack}
            className="text-[#4D5CEC] text-sm font-medium hover:text-[#3D4CDC] flex items-center gap-1"
          >
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-3xl font-bold ${getRankColor(rank)}`}>{rank}</span>
                </div>
              </div>
              ...
              <div className="relative h-8 rounded-full overflow-hidden mb-2">
                ...
                <div className="absolute left-0 top-0 h-full bg-gray-200" style={{ width: `${scoreData.passScore}%` }} />
                ...
                <div
                  className={`absolute left-0 top-0 h-full transition-all duration-500 ${
                    isPassed ? 'bg-gradient-to-r from-green-400 to-emerald-500' : 'bg-[#4D5CEC]'
                  }`}
                  style={{ width: `${scoreData.score}%` }}
                />
                {/* Pass Line */}
                <div className="absolute top-0 h-full w-1 bg-green-600 z-10" style={{ left: `${scoreData.passScore}%` }} />
                {/* Clear Label */}
                <div
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="flex items-center justify-center">
              <svg viewBox="0 0 400 400" className="w-full max-w-sm">
                ...
                {/* Labels */}
                {['課題理解力', '思考力', '行動力', '成長力', '協働力'].map((label, i) => {
                  const angle = (i * 72 - 90) * (Math.PI / 180);
                ...
                <div key={index} className="border-b border-gray-100 pb-4 last:border-0">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500">({criteria.definition})</span>
                    </div>
                    <span className="text-[#4D5CEC] font-bold">{criteria.score}点</span>
                  </div>
                  <div className="h-2 bg-gray-200 rounded-full mb-2">
                    <div className="h-full bg-[#4D5CEC] rounded-full" style={{ width: `${criteria.score}%` }} />
                  </div>
                ...
                {msg.role === 'interviewer' ? (
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full flex-shrink-0 overflow-hidden">
                      <img src="/FastPass_result_cahticon.png" alt="AI" className="w-full h-full object-cover" />
                    </div>
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    >
                      <p className={`text-sm ${msg.feedback?.isHighlight ? 'text-gray-700' : ''}`}>{msg.message}</p>
                    </div>
                    {msg.feedback && (
